<template>
  <v-container class="mt-2">
    ...
    >
      <h2>Nota social</h2>
      <v-spacer></v-spacer>
      <v-btn color="info" to="valoracionpsi" dark small> Nueva Nota </v-btn>
    </v-toolbar>
    <v-card>
      <v-card flat>
        <v-row>
      ...
      >
        <template v-slot:item="{ item }">
          <tr v-on:click.prevent.stop="editar(item)">
            <td>{{ item.idnotasocial }}</td>
            <td>{{ item.fecha }}</td>
            <td>{{ item.evolucion }}</td>
            <td>{{ item.edoactual }}</td>
            <td>{{item.tsocial}}</td>
          </tr>
        </template>
      </v-data-table>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Estado actual", value: "edoactual" },
        { text:'Trab. social', value: "tsocial"}
      ],
      notasocial: [],
    };
  },
  ...
  methods: {
    ...
    consultar() {
      ...
      const cUrlbase = "notasocial.get.all.pac?";
        ...
        .then((response) => {
          ...
          if (response.data.error === null) {
            if (response.data.result != null) {
              this.notasocial = response.data.result;
            } else {
              this.showSnack({
    ...
    editar(item) {
      ...
      this.$router
        .push({ name: "valoracionpsi", params: item })
        .catch(() => {});
    },
  },
};
