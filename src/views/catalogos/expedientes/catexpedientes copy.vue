<template>
  <v-container class="mt-3">
    <v-row>
      <v-col>
        <v-card elevation="0">
          <v-toolbar dark dense flat color="black">
            <v-col cols="1">
              <v-avatar
                tile
                dense
            ...
            <h3 class="ml-3">Catálogo de expedientes</h3>
            <v-spacer />
            <v-col cols="3">
              <v-text-field
                v-model="search"
          ...
          >
            <template v-slot:item="{ item }">
              <tr @click.stop.prevent="editar(item)">
                ...
                <td v-if="item.imagenog">
                  <img
                    :src="item.imagenog"
                    style="max-height: 40px; width: auto"
                  />
                </td>
                <td v-else><img :src="logo" style="max-height: 20px" /></td>
                <!-- <td class="red--text">
                  {{ item.idblog }}
                </td> -->
...
export default {
  data() {
    return {
      ...
      headers: [
        { text: "Id", value: "titulo" },
        { text: "Foto", value: "imagenog", align: "start" },
        // { text: 'Id', value: 'idblog', align: 'start', color:"red", sortable: false, },
        { text: "Nombre", align: "start" },
        { text: "Ciudad", value: "empresa" },
        { text: "Pais", value: "iso3" },
        { text: "Fecha Alta", value: "fechaalta" },
        { text: "Id C. Atn", value: "idcatencion" },
      ],
      json_fields: {
        Id: "idcita",
        Nombre: "idcontacto",
      ...
      this.$http
        ...
        .then((response) => {
          if (response.data.error === null) {
            ...
            //   // element.hora = element.fechaalta.substr(11,17)
            //   element.fechaalta = element.fechaalta.substr(0, 10);
            //   let cFecha = new Date(element.fechaalta).toLocaleDateString(
            //     "es-MX",
            //     {
            //       timeZone: "UTC",
            //       month: "short",
